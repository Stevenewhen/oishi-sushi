...
const MAPS_PLACE_URL =
  PLACE_ID ? `https://www.google.com/maps/place/?q=place_id:${encodeURIComponent(PLACE_ID)}` : "";
  ...
  return (
    <div className="flex items-end leading-none select-none shrink-0">
      <Image
        ...
        alt="Oishi"
        width={128}               // pick a sensible intrinsic size
        height={32}
        className="h-7 w-auto sm:h-8 shrink-0"
        priority                   // keep it snappy in header
      />
    </div>
  );
}
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    <html lang="en">
      <body className={["min-h-dvh bg-waves text-white antialiased", sushi.variable].join(" ")}>
          ...
          <div className="mx-auto max-w-6xl px-4 h-14 flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              <nav className="hidden sm:flex items-center gap-4 text-sm min-w-0">
                <Link href="/menu" className="link-nav">Menu</Link>
                <a href={MAPS_URL} target="_blank" rel="noopener noreferrer" className="link-nav">
                  Directions
                </a>
              </nav>
              <a href={TEL} className="btn btn--sm btn--primary">Call {(PHONE)}</a>
            </div>
          </div>
        </header>
